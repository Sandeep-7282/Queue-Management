...
async function generateQueueNumber(name, email, mobileNumber, serviceType) {
  try {
    ...
    const queueLength = await Customer.count({ where: { serviceType } });
...
app.post('/queue', async (req, res) => {
  try {
    ...
    res.status(200).json({message:"You Have Successfully Registered for the Queue"});
  } catch (error) {
    console.error('Error generating queue number:', error); 
    statusCode = 400;
...
app.post('/admin/next', async (req, res) => {
  try {
    ...
    await sendEmailToCustomer(nextCustomer);
...
async function checkWaitingTimeAndSendEmails() {
  try {
    ...
    if (!customers || customers.length === 0) {
      console.log('No customers with waiting time exceeding 0 minutes.');
      return;
    }

    for (const customer of customers) {
      const { waitingTime, email } = customer;
      const newWaitingTime = waitingTime -0;


      await Customer.update({ waitingTime: newWaitingTime }, { where: { email } });

 
      await sendEmailToCustomer(customer);
    }
  } catch (error) {
    console.error('Error checking waiting time and sending emails:', error);
  }
}
